...
> {
  ...
  override render() {
    return html`
      <uui-dialog-layout headline=${this._isBulk
        ? `Create redirects for ${this._items.length} pages?`
        : "Create a redirect for this page?"}>
        ${this.#renderBody()}
  ...
  #submitLabel(): string {
    ...
    if (count === 0) return "Trash";
    if (count === 1) return "Trash and create redirect";
    return `Trash and create ${count} redirects`;
  }
  ...
  #renderBody() {
    ...
    if (this._loadFailed) {
      return html`<p class="muted">Suggestions could not be loaded. You can still trash without creating redirects.</p>`;
    }

    if (this._items.length === 0) {
      return html`<p class="muted">No redirect suggestions were found for the selected content.</p>`;
    }
  ...
  #renderSingle(item: SmartRedirectSuggesterModalItem) {
    ...
    return html`
      <p>
        <strong>${item.name}</strong> will be moved to the recycle bin.
      ...
      ${item.candidates.length > 0
        ? html`
          ...
          `
        : html`<p class="muted">No automatic suggestions found.</p>`}

      <div class="picker-actions">
        <uui-button
          label=${isManualSelection ? "Change picked content" : "Pick content"}
          look="secondary"
      ...
      ${isManualSelection
        ? html`
            <div class="manual-selection">
              ...
              <uui-ref-node name=${manualDisplay?.name ?? current ?? ""} detail=${manualDisplay?.url || "Manually picked"} readonly>
                <umb-icon slot="icon" name="icon-document"></umb-icon>
              </uui-ref-node>
            </div>
          `
      ...
      ${totalPages > 1
        ? html`
            <div class="pagination">
              ...
              ></uui-button>
              <span class="muted">Page ${this._currentPage} of ${totalPages}</span>
              <uui-button
                compact
                label="Next page"
                look="secondary"
  ...
  #renderBulkRow(item: SmartRedirectSuggesterModalItem) {
    ...
    return html`
      <uui-table-row>
        <uui-table-cell>
          <uui-ref-node name=${item.name} detail=${item.oldUrls[0]?.url || "Trashed content"} readonly>
            <umb-icon slot="icon" name="icon-document"></umb-icon>
          </uui-ref-node>
        </uui-table-cell>
        <uui-table-cell>
          ${current === null
            ? html`<span class="muted">No redirect</span>`
            : html`<uui-ref-node name=${display?.name ?? current ?? ""} detail=${display?.url || "Selected content"} readonly>
                <umb-icon slot="icon" name="icon-document"></umb-icon>
              </uui-ref-node>`}
        </uui-table-cell>
        <uui-table-cell>
          ...
          ${manualDisplay
            ? html`
                <div class="manual-selection">
                  ...
                  <uui-ref-node name=${manualDisplay.name} detail=${manualDisplay.url || "Manually picked"} readonly>
                    <umb-icon slot="icon" name="icon-document"></umb-icon>
                  </uui-ref-node>
                </div>
              `
